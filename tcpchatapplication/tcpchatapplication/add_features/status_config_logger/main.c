...
typedef struct {
    ...
    int is_status; // 0 = normal client, 1 = status client
} client_t;
...
{
    char count_str[32];
    snprintf(count_str, sizeof(count_str), "Active clients: %d\n", count);

    //    for(int i = 0; i < MAX_CLIENT; i++) {
    //        if(clients[i].fd != -1) {
    //            send(clients[i].fd, count_str, strlen(count_str), 0);
    //        }
    //    }
    send(status, count_str, strlen(count_str), 0);
}
...
void num_to_string(int num, char* str, size_t size) { snprintf(str, size, "%d", num); }
        ...
        for(int i = 0; i < MAX_CLIENT; i++) {
            if(clients[i].fd != -1 && FD_ISSET(clients[i].fd, &readfd)) {
                ...
                if(n <= 0) {
                    ...
                    // handle "status" at any time
                    if(clients[i].named == 0) {
                        buffer[strcspn(buffer, "\n")] = '\0'; // strip newline
                        strncpy(clients[i].name, buffer, NAME_LEN - 1);

                        if(strcmp(buffer, "status") == 0) {
                            clients[i].is_status = 1;
                            status_fd = clients[i].fd; // remember who is status
                        } else {
                            clients[i].is_status = 0;
                        }
                        ...
                        if(strncmp(buffer, "exit", 4) == 0) {
                            ...
                            for(int j = 0; j < MAX_CLIENT; j++) {
                                if(clients[j].fd != -1 && clients[j].fd != master_socket &&
                                   clients[j].fd != clients[i].fd && clients[j].is_status == 0) // skip status clients
                                {
