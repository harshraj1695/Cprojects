...
int main() {
    ...
    fcntl(socket_conn, F_SETFL, flags | O_NONBLOCK);

    for (;;) {
        printf("Waiting for accept...\n");
        // block until a client connects (we’ll poll manually)
        while ((data_socket = accept(socket_conn, NULL, NULL)) == -1) {
        ...
        while (1) {
            // check if another client tries to connect
            int new_client = accept(socket_conn, NULL, NULL);
            if (new_client >= 0) {
                // second client — send BUSY
                strcpy(buffer, "BUSY");
                write(new_client, buffer, strlen(buffer) + 1);
                close(new_client);
                printf("Rejected extra client: sent BUSY\n");
            }
